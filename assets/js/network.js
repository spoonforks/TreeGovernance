/* Full network projection of the shared evidence model. */
(async function () {
'use strict';
try {
const MODEL=await window.TREE_GOV_READY;
...
const colors={actor:'#78978e',process:'#c6a262',data:'#8094ad',rule:'#ae7770',ecology:'#7f9f6a',resource:'#9b8aa8'};
...
function nodeRadius(n){return {actor:42,process:67,data:52,rule:62,ecology:54,resource:64}[n.type]||45}
...
function concentricLayout(){
  const ns=visibleNodes();const by={rule:[],actor:[],process:[],data:[],ecology:[],resource:[]};ns.forEach(n=>(by[n.type]??by.process).push(n));
  const spec=[['ecology',100],['data',220],['process',355],['actor',490],['rule',610],['resource',735]],cx=520,cy=390;
...
function shapeElement(n){
  ...
  else if(n.type==='resource'){s=document.createElementNS(NS,'rect');s.setAttribute('x','-56');s.setAttribute('y','-32');s.setAttribute('width','112');s.setAttribute('height','64');s.setAttribute('rx','4')}
  ...
  if(n.type==='resource'){const line=document.createElementNS(NS,'path');line.setAttribute('d','M-53 -25H53 M-53 25H53');line.setAttribute('class','resource-line');g.appendChild(line)}
  g.setAttribute('role','button');g.setAttribute('tabindex','0');g.setAttribute('aria-label',n.label);
...
svg.addEventListener('pointerup',ev=>{
  ...
  if(didDrag){
    ...
    if(clickNodeId){ev.preventDefault();state.suppressClick=true;selectNode(clickNodeId);setTimeout(()=>state.suppressClick=false,90)}
  }
});
...
function selectNode(id){if(!nodeById[id])return;if(!visibleNodeIds().has(id)){state.lens='all';state.focusIds=null;document.getElementById('lens').value='all';runLayout()}state.selectedNode=id;state.selectedEdge=null;state.tab='details';syncTabs();render();showNode(id);syncNetworkContext()}
function selectEdge(id){if(!edgeById[id])return;const chosen=edgeById[id],visible=visibleNodeIds();if(!visible.has(chosen.source)||!visible.has(chosen.target)){state.lens='all';state.focusIds=null;document.getElementById('lens').value='all';runLayout()}state.selectedEdge=id;state.selectedNode=null;state.tab='details';syncTabs();render();showEdge(id);syncNetworkContext()}
function clearHighlight(){state.selectedNode=null;state.selectedEdge=null;state.hoverNode=null;state.focusIds=null;state.pathIds.clear();state.pathEdgeIds.clear();state.searchHits.clear();render();if(state.tab==='details')showGuide();syncNetworkContext()}
...
function showNode(id){
 ...
 const ta=document.getElementById('nodeNote'),fl=document.getElementById('flagNode'),save=()=>{state.notes[id]={text:ta.value,flagged:fl.checked};try{localStorage.setItem('treeGovNotesV1',JSON.stringify(state.notes))}catch{ta.setAttribute('title','Browser storage unavailable; copy this note before leaving.')}};ta.oninput=save;fl.onchange=save;
}
...
function showGuide(){sideBody.innerHTML=`<div class="kicker">How to use this prototype</div><h2>One system, two perspectives</h2><p>This graph and Three-level use the same elements, relations and sources. Select an element, then switch modes to continue exploring it. Your network layout is kept when you switch back in this tab.</p><p>The map separates <strong>actors</strong>, <strong>processes</strong>, <strong>data/objects</strong>, <strong>rules</strong>, <strong>resources</strong> and <strong>ecological outcomes</strong>. The point is to expose mechanisms that a stakeholder-only map tends to hide.</p>
...
function showPath(){
 ...
 sideBody.innerHTML=`<div class="kicker">Path explorer</div><h2>Trace a route through the system</h2><p>Find a directed mapped path between two visible elements, respecting arrow direction. This is structural, not causal or legal proof.</p><div class="pathbox"><select id="pathSource">${opts}</select><select id="pathTarget">${opts}</select><button type="button" class="primary" id="tracePath">Trace</button></div><div id="pathResult" class="empty">Choose two elements.</div>
...
function tracePath(s,t){
 const ids=visibleNodeIds(),es=visibleEdges(),adj=Object.fromEntries([...ids].map(id=>[id,[]]));es.forEach(e=>{adj[e.source].push([e.target,e.id])});
...
document.getElementById('search').oninput=e=>{const q=e.target.value.trim().toLowerCase();state.searchHits.clear();if(q)visibleNodes().forEach(n=>{if([n.label,n.translation,n.id].join(' ').toLowerCase().includes(q)||(n.summary||'').toLowerCase().includes(q)||(n.group||'').toLowerCase().includes(q))state.searchHits.add(n.id)});render();if(state.searchHits.size)fitVisible(state.searchHits)};
document.getElementById('exportBtn').onclick=()=>{const out=JSON.parse(JSON.stringify(MODEL));const blob=new Blob([JSON.stringify(out,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='tree-governance-model-v0.4.json';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000)};
// Capture visual state separately from the analytical model. A plain visit to
// index.html always opens the network; only a mode switch restores tab state.
function snapshotNetwork(){
 return {lens:state.lens,layout:document.getElementById('layout').value,view:{...state.view},
 positions:Object.fromEntries(Object.entries(runtime).map(([id,p])=>[id,{x:p.x,y:p.y}])),
 focus:state.focusIds?[...state.focusIds]:null,selectedNode:state.selectedNode,
 relationLabels:state.relationLabels,search:document.getElementById('search').value};
}
function syncNetworkContext(){
 const e=edgeById[state.selectedEdge],node=state.selectedNode||e?.source;
 const p=new URLSearchParams(location.hash.slice(1));
 p.set('view','network');p.set('lens',state.lens);p.set('layout',document.getElementById('layout').value);
 if(node)p.set('node',node);else p.delete('node');
 if(e)p.set('edge',e.id);else p.delete('edge');
 p.set('basis',state.documentedOnly?'documented':'all');
 history.replaceState(null,'','#'+p);
 TreeGovShell.publish({node:node||null,edge:e?.id||null,arena:nodeById[node]?.home,basis:state.documentedOnly?'documented':'all'});
}
function restoreNetwork(){
 const p=new URLSearchParams(location.hash.slice(1)),saved=TreeGovShell.restore();
 const lens=p.get('lens')||saved?.lens||'all';
 state.lens=[...document.getElementById('lens').options].some(o=>o.value===lens)?lens:'all';
 const layout=p.get('layout')||saved?.layout||'organic';
 document.getElementById('layout').value=['organic','flow','concentric'].includes(layout)?layout:'organic';
 document.getElementById('lens').value=state.lens;
 state.documentedOnly=p.get('basis')==='documented';document.getElementById('documentedOnly').checked=state.documentedOnly;
 if(saved){
  state.relationLabels=!!saved.relationLabels;document.getElementById('labels').checked=state.relationLabels;
  document.getElementById('search').value=typeof saved.search==='string'?saved.search:'';
  if(Array.isArray(saved.focus))state.focusIds=new Set(saved.focus.filter(id=>nodeById[id]));
 }
 const node=nodeById[p.get('node')]?p.get('node'):null;
 let sameLayout=!!saved&&saved.lens===state.lens;
 if(node&&!visibleNodeIds().has(node)){
  state.focusIds=null;
  if(!lensAllows(nodeById[node])){state.lens='all';document.getElementById('lens').value='all';sameLayout=false;}
 }
 initPositions();
 if(saved?.positions&&sameLayout){
  for(const [id,pos] of Object.entries(saved.positions))if(runtime[id]&&Number.isFinite(pos.x)&&Number.isFinite(pos.y))Object.assign(runtime[id],pos);
  render();
 }else runLayout(false);
 if(saved?.view&&sameLayout&&['x','y','k'].every(k=>Number.isFinite(saved.view[k]))&&saved.view.k>=.12&&saved.view.k<=3){state.view={...saved.view};applyView();}
 else fitVisible();
 if(node){state.selectedNode=node;state.tab='details';syncTabs();render();showNode(node);}
 else showGuide();
 const edge=edgeById[p.get('edge')];
 if(edge&&(!state.documentedOnly||edge.basis==='documented'))selectEdge(edge.id);
 // A different element selected in Three-level should be brought into view.
 if(node&&saved&&node!==saved.selectedNode)fitVisible(neighborIds(node,1));
 const query=document.getElementById('search').value.trim().toLowerCase();
 if(query){for(const n of visibleNodes())if([n.label,n.summary,n.translation,n.id].join(' ').toLowerCase().includes(query))state.searchHits.add(n.id);updateClasses();}
 syncNetworkContext();
}
TreeGovShell.register(snapshotNetwork);
new ResizeObserver(()=>applyView()).observe(svg);
restoreNetwork();
document.getElementById('modelStatus').textContent=`${MODEL.nodes.length} elements / ${MODEL.edges.length} relations`;
['lens','layout','documentedOnly','labels'].forEach(id=>document.getElementById(id).addEventListener('change',()=>{
 if((state.selectedNode&&!visibleNodeIds().has(state.selectedNode))||(state.selectedEdge&&!visibleEdges().some(e=>e.id===state.selectedEdge))){state.selectedNode=null;state.selectedEdge=null;showGuide();updateClasses();}
 syncNetworkContext();
}));
document.getElementById('reset').addEventListener('click',()=>syncNetworkContext());
window.addEventListener('hashchange',restoreNetwork);
window.TreeGovernance.network={selectNode,getState:()=>snapshotNetwork()};
}catch(err){
 console.error(err);
 ...
 document.getElementById('sideBody').replaceChildren(message);
}
})();
